...
class $modify(GDO_LocalLevelManager, LocalLevelManager)
{
	$override gd::string getMainLevelString(int id)
	{
		...
		auto file = Mod::get()->getSettingValue<bool>("empty-levels") ? CCString::create("base.txt"_spr) : (Mod::get()->getSettingValue<bool>("updated-levels")) ? CCString::createWithFormat("level-%iB.txt"_spr, id) : CCString::createWithFormat("level-%i.txt"_spr, id);
