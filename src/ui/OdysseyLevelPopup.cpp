...
void OdysseyLevelPopup::onPlay(CCObject *sender)
{
    ...
    if (!MusicDownloadManager::sharedState()->isSongDownloaded(Odyssey::getLevelSongID(m_levelID)))
    {
        auto content = Odyssey::createText(
            "Are you sure you want to play the level without <cj>Audio assets</c>? You should download them before playing.\n<cy>They can be downloaded in the level options</c> (Bottom left button)",
            "Estas seguro que quieres jugar el nivel sin sus <cj>Assets de Audio</c>? Deberias descargalos antes de jugar. <cy>Se pueden descargar en las opciones del nivel</c> (Boton abajo a la izquierda)."
        );

        auto popup = createQuickPopup(
            "No Audio",
            content,
            Odyssey::createText("Go back", "Volver").c_str(),
            Odyssey::createText("Play", "Jugar").c_str(),
    ...
    void OdysseyLevelPopup::onLore(CCObject * sender)
    {
        if (m_levelID == 7501)
        {
            auto lore = Odyssey::createText(
                "<cg>\"Conclusive Journey\"</c> tells the tale of an ancient road leading to the desolate desert. Before the events of <cy>GD: Odyssey</c>, the team of adventurers wander here in search for more clues to help understand what's happening in Geometry City.",
                "<cg>\"Conclusive Journey\"</c> cuenta la historia de un antiguo camino que conduce al desolado desierto. Antes de que ocurran los eventos de <cy>GD: Odyssey</c>, el equipo de aventureros deambula por aqui en busca de mas pistas que ayuden a comprender lo que esta ocurriendo en Geometry City.");
        ...
        {
            auto lore = Odyssey::createText(
                "<cg>\"Burning Sands\"</c> tells the story of Nefferkitty, an ancient Egyptian god and protector of the Stone of elements. Before the events of <cy>GD: Odyssey</c>, the team of adventurers visited the desert during the <cr>Wanderer's</c> invasion. The events of this game take place after completing this level.",
