...
class $modify(OdysseyShopLayer, GJShopLayer)
{
    ...
    //  If the player enters the spot for the first time, activates this special sequence
    void getCarpDialog()
    {
    ...
    //  Replaces the afford message with custom ones (overwrites the dialog)
    void showCantAffordMessage(GJStoreItem *item)
    {
        std::vector<gd::string> dialogs = {
            Odyssey::createText("Ya know you <co>can't afford that</c>, right?", "Sabes que <co>no puedes pagarlo</c>, verdad?"),
            Odyssey::createText("You miss some <cl>Mana orbs</c>, pal.", "Te faltan <cl>Orbes</c>, amigo."),
            Odyssey::createText("<cg>Collect</c> more <cl>Mana Orbs</c>.", "<cg>Colecciona</c> mas <cl>Orbes</c>."),
            Odyssey::createText("You <co>do not</c> have enough <cl>Mana Orbs</c>.", "Tu <co>no tienes</c> suficientes <cl>Orbes</c>."),
            Odyssey::createText("Come back when you're a little... <cl>Richer</c>!", "Vuelve cuando tengas mas... <cl>Mana</c>!"),
            Odyssey::createText("See the <cy>TV</c> next to me? Maybe you should use it.", "Ves la <cy>TV</c> al lado mio? Tal vez deberias usarlo.")
        };
        ...
        int rand1 = (std::rand() % dialogs.size());
        int rand2 = (std::rand() % sprites.size());

        auto dialogObject = DialogObject::create("Carp", dialogs.at(rand1), sprites.at(rand2), 1, false, {255, 255, 255});

        auto dialogLayer = DialogLayer::createDialogLayer(dialogObject, nullptr, 2);
        dialogLayer->animateInRandomSide();
        dialogLayer->setZOrder(10);
    
        this->addChild(dialogLayer, 200);
    }
};
