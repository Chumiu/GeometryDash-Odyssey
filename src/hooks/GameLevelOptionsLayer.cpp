    ...
    static GameLevelOptionsLayer *create(GJGameLevel *p0)
    {
        if (p0->m_levelType == GJLevelType::Main)
        {
            ...
            auto infoButton = InfoAlertButton::create(
                spanish ? "Porque este cambio?" : "Why this change?",
                spanish ? "El mod anteriormente tuvo <cr>problemas de optimizacion</c>, esto ayuda a reducir el peso del mod haciendo que descargues el <cj>Audio</c> como si fuera un <cy>Nivel Online</c>." : "The mod previously had <cr>optimization issues</c>, this helps drastically decrease the mod's size by making you download the <cj>Audio Assets</c> like an <cy>Online level</c>.",
                0.5);
